<section class="text-gray-600">
    <div class="container px-5 py-20 mx-auto">
        <div class="lg:w-2/4 w-full mx-auto overflow-auto">
            ...
            <form enctype="multipart/form-data" method="POST" action="/admin/create">
                @csrf
                <div class="flex flex-wrap">
                    <div class="p-2 w-1/2">
                        <div class="relative">
                            <label for="name" class="leading-7 text-sm text-gray-600">Nome do livro</label>
                            <input type="text" id="name" value="{{ old('name') }}" name="name" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                            @error('name')
                    ...
                    <div class="p-2 w-1/2">
                        <div class="relative">
                            ...
                            <input type="url" id="cover" name="cover" value="{{ old('cover') }}"
                                class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" />
                            @error('cover')
                                <div class="text-red-400 text-sm">{{ $message }}</div>
                            @enderror
