    ...
    <div class="lg:w-2/3 w-full mx-auto overflow-auto">
        ...
        <table class="table-auto w-full text-left whitespace-no-wrap">
            ...
            @foreach ($books as $book)
                ...
                <td class="px-4 py-3">
                    <a href="/admin/{{ $book->slug }}"><img alt="cover" class="object-cover object-center w-full h-full block" src="{{  $book->cover }}"></a>
                </td>
                <td class="px-4 py-3"><a href="/admin/{{ $book->slug }}">{{ $book->name }}</a></td>
                <td class="px-4 py-3">R${{ $book->price }}</td>
                <td class="px-4 py-3">{{ $book->stock }}</td>
                <td class="px-4 py-3 text-sm text-right space-x-3 text-gray-900">
                    <a class="mt-3 text-indigo-500 inline-flex items-center" href=" admin/{{ $book->id }}/edit">Editar</a>
